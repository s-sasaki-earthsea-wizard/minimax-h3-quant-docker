...
.PHONY: gen
gen: | .env ## Headless video: make gen PROMPT="..." [IMAGE=path] [DURATION=5] [SEED=n] [TURBO=4|8]
	@test -n "$(PROMPT)" || { echo 'usage: make gen PROMPT="..." [IMAGE=path] [DURATION=5] [SEED=n] [TURBO=4|8]'; exit 1; }
	$(RUN) python3 scripts/generate.py --prompt "$(PROMPT)" --duration "$(DURATION)" \
		$(if $(IMAGE),--image "$(IMAGE)") $(if $(UPLOAD_ALWAYS),--upload-always) \
		$(if $(SEED),--seed "$(SEED)") $(if $(TURBO),--turbo "$(TURBO)") \
		--server "$(COMFY_SERVER)"

.PHONY: gen-t2v
gen-t2v: | .env ## Prompt -> Ollama -> video: make gen-t2v PROMPT="..." [SPEECH=ja] [DURATION=5] [SEED=n] [TURBO=4|8] [MODEL=...] [DRY_RUN=1]
	@test -n "$(PROMPT)" || { echo 'usage: make gen-t2v PROMPT="..." [SPEECH=ja] [DURATION=5] [SEED=n] [TURBO=4|8] [MODEL=...] [DRY_RUN=1]'; exit 1; }
	...
	$(RUN) python3 scripts/pipeline.py "$(PROMPT)" --model "$(MODEL)" --duration "$(DURATION)" \
		...
		$(if $(SEED),--seed "$(SEED)") $(if $(TURBO),--turbo "$(TURBO)") $(if $(DRY_RUN),--dry-run) \
		--comfy-server "$(COMFY_SERVER)"

.PHONY: gen-i2v
gen-i2v: | .env ## Still + prompt -> Ollama -> video: make gen-i2v IMAGE=path [PROMPT="..."] [SPEECH=ja] [IMAGE_PROMPT="..."] [DURATION=5] [SEED=n] [TURBO=4|8] [DRY_RUN=1]
	@test -n "$(IMAGE)" || { echo 'usage: make gen-i2v IMAGE=path [PROMPT="..."] [SPEECH=ja] [IMAGE_PROMPT="..."] [DURATION=5] [SEED=n] [TURBO=4|8] [DRY_RUN=1]'; exit 1; }
...
pipeline: | .env ## Same, with the mode taken from IMAGE=: make pipeline THEME="..." [IMAGE=path] [SPEECH=ja] ...
	@test -n "$(THEME)$(IMAGE)" || { echo 'usage: make pipeline THEME="..." [IMAGE=path] [IMAGE_PROMPT="..."] [SPEECH=ja] [MODEL=...] [DURATION=5] [SEED=n] [TURBO=4|8] [DRY_RUN=1]'; exit 1; }
